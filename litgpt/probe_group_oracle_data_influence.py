...
def main(
    ...
    if False:
        train_dataset_1 = StreamingDataset(
            input_dir=str(base_dir) + "/data/fineweb/sample-100BT/val",
            item_loader=TokensLoader(block_size=model.max_seq_length + 1),
            drop_last=True,
        )
        # 24484
        train_dataset_2 = StreamingDataset(
            input_dir=str(base_dir) + "/data/fineweb-edu/sample-100BT/val",
            item_loader=TokensLoader(block_size=model.max_seq_length + 1),
            drop_last=True,
        )
        train_dataset = CombinedStreamingDataset(
            datasets=[train_dataset_1, train_dataset_2],
            seed=42,
        )
    ...
    if data:
        ...
        val_dataloader_1, val_dataloader_2 = fabric.setup_dataloaders(
    ...
    oracle = []
    f = open("/data/users/zichunyu/galactic/src/my_dataset-100BT.jsonl")
    train_dataset = [json.loads(line) for line in f]
    cluster_num = max([data["__cluster"] for data in train_dataset]) + 1
    for i in tqdm(range(cluster_num)):
        ...
        scores = fit(
            ...
            torch.tensor(
                [data["input_ids"] for data in train_dataset if data["__cluster"] == i]
            ),
            val_dataloaders,
            out_dir,
            tokenizer_dir,
    ...
    torch.save(oracle, "/data/users/zichunyu/galactic/src/oracle-100BT-1.pt")
...
def fit(
    ...
    print(evaluate(fabric, model, val_dataloaders))
    exit(0)
...
def evaluate(fabric, model, val_dataloaders):
    ...
    for val_dataloader in val_dataloaders:
        ...
        for batch in val_dataloader:
            ...
            # train_set.append(
            #     {
            #         "input_ids": input_ids[0][:1024].cpu().numpy().tolist(),
            #         "labels": labels[0][:1024].cpu().numpy().tolist(),
            #     }
            # )
            logits = model(input_ids)
    ...
    # torch.save(train_set, "../data/flan/train.pt")
    model.train()
    return losses
