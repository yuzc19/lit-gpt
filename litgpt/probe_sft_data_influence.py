...
def main(
    ...
    train_dataloader = DataLoader(
        torch.load(base_dir / "data/flan/train.pt")[rank * 1000 : ((rank + 1) * 1000)],
        batch_size=1,
    ...
    for train_data in tqdm(train_iterator):
        ...
        oracle.append(
            {
                "input_ids": train_data["input_ids"][0].cpu().numpy().tolist(),
                "scores": scores,
            }
        )
        if cnt == 625:
            ...
            if "/mnt" not in str(out_dir):
                processed_ds.save_to_disk(
                    f"out/step-{train.resume_steps}-sft/{config.name}/{rank}"
                )
            else:
                processed_ds.save_to_disk(
                    f"/mnt/mffuse/scaling_mates/tc_out/step-{train.resume_steps}/{config.name}/{rank}"
                )
            exit(0)
